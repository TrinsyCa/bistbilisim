      ...
      <div class="row">
         ...
         <?php
            if(isset($_POST["resimyukle"]))
            {
               ...
               $resimekle = $db->prepare("INSERT INTO gallery SET name = ?");
               $resimekle->execute([$resimad]);
               header("Refresh: 0; url=gallery.php");
            }

            $resimdb = $db->prepare("SELECT * FROM gallery ORDER BY id DESC");
            $resimdb->execute();
            ...
            foreach ($resimler as $row)
            {
               ...
               <a target="_blank" href="../img/pictures/'.$row["name"].'"><img src="../img/pictures/'.$row["name"].'"></a>
               </div>';
            }
         ?>
         </div>
...
<script>
   function sil(sutunId , sutunName) {
      if (confirm("Bu resmi silmek istediğinize emin misiniz?")) {
         $.post("cmd/silpicture.php", { sutunId: sutunId , sutunName: sutunName})
